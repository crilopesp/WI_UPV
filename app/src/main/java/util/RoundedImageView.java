...
/**
 * Created by Cristian on 19/08/2014.
 */
public class RoundedImageView extends ImageView {
    ...
    public void onDraw(Canvas canvas) {
        ...
        if (hasSelector && isSelected) { // Draw the selector stroke & apply the selector filter, if applicable
            ...
            final Rect rect = new Rect(0, 0, canvas.getWidth() + outerWidth, canvas.getHeight() + outerWidth);
            final RectF rectF = new RectF(rect);
            final float roundPx = 10;
            canvas.drawRoundRect(rectF, roundPx, roundPx, paintBorder);
